...
void test()
{
    ...
    for(int i=0;i<10;i++)
    {
       tmp[i] = sin(i * 6.28 / 10)*100;
    }
    static int index = 0;
    m_wpv4.arMotorPower[0] = tmp[index];
    index ++;
    if(index >= 10)
      index = 0;
    data_recv = 10;
...
int main(int argc, char** argv)
{
    ...
    ros::Subscriber sub_imu = n.subscribe("/imu/data", 10, IMUCallback);
    ...
    {
        ROS_WARN("[设置]底盘类型= %s",chassis_type.c_str());
        if(chassis_type == "4wd")
        {
            ROS_WARN("[设置]四轮驱动底盘");
            nChassisType = CT_4WD;
    ...
    std_msgs::Float64MultiArray motor_power_msg;
    bool bMotorPower = true;
    n_param.param<bool>("motor_power", bMotorPower, false);
    if(bMotorPower == true)
    {
        ROS_INFO("开启 电机功率反馈");
        motor_power_pub = n.advertise<std_msgs::Float64MultiArray>("motor_power",10);
        motor_power_msg.data.resize(4);
    }

    ros::Publisher motor_speed_pub;
    std_msgs::Int64MultiArray motor_speed_msg;
    bool bMotorSpeed = true;
    n_param.param<bool>("motor_speed", bMotorSpeed, false);
    if(bMotorSpeed == true)
    {
        ROS_INFO("开启 电机转速反馈");
        motor_speed_pub = n.advertise<std_msgs::Int64MultiArray>("motor_speed",10);
        motor_speed_msg.data.resize(4);
    }

    ros::Publisher motor_torque_pub;
    std_msgs::Float64MultiArray motor_torque_msg;
    bool bMotorTorque = true;
    n_param.param<bool>("motor_torque", bMotorTorque, false);
    if(bMotorTorque == true)
    {
        ROS_INFO("开启 电机扭矩反馈");
        motor_torque_pub = n.advertise<std_msgs::Float64MultiArray>("motor_torque",10);
        motor_torque_msg.data.resize(4);
    }
    ...
    while(n.ok())
    {
        ...
        // 发送电机功率
        if(bMotorPower == true && data_recv > 0)
        {
            double kPower = 0.1; // 返回值单位为0.1瓦，乘上系数0.1，转换成单位：瓦
            motor_power_msg.data[0] = (double)m_wpv4.arMotorPower[0] * kPower;
            motor_power_msg.data[1] = (double)m_wpv4.arMotorPower[1] * kPower;
            motor_power_msg.data[2] = (double)m_wpv4.arMotorPower[2] * kPower;
            motor_power_msg.data[3] = (double)m_wpv4.arMotorPower[3] * kPower;
            motor_power_pub.publish(motor_power_msg);
        }

        // 发送电机速度
        if(bMotorSpeed == true && data_recv > 0)
        {
            // 返回值单位为转/分
            motor_speed_msg.data[0] = m_wpv4.arMotorSpeed[0];
            motor_speed_msg.data[1] = m_wpv4.arMotorSpeed[1];
            motor_speed_msg.data[2] = m_wpv4.arMotorSpeed[2];
            motor_speed_msg.data[3] = m_wpv4.arMotorSpeed[3];
            motor_speed_pub.publish(motor_speed_msg);
        }

        // 发送电机扭矩
        if(bMotorTorque == true && data_recv > 0)
        {
            double kTorque = 0.01 * 20 * 0.0914;  // 返回值单位为10mA，20为常量系数，0.0914为电机的扭矩系数
            motor_torque_msg.data[0] = (double)m_wpv4.arMotorCurrent[0] * kTorque;
            motor_torque_msg.data[1] = (double)m_wpv4.arMotorCurrent[1] * kTorque;
            motor_torque_msg.data[2] = (double)m_wpv4.arMotorCurrent[2] * kTorque;
            motor_torque_msg.data[3] = (double)m_wpv4.arMotorCurrent[3] * kTorque;
            motor_torque_pub.publish(motor_torque_msg);
        }
