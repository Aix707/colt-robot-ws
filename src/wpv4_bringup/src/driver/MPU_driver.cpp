...
double CMPU_driver::m_CalQuaternionVal(unsigned char *inBuf)
{
	int nTmp = m_Piece2int(inBuf);
	double retVal = (double)nTmp ;
	return retVal;
}
...
double Short2Double(int inVal)
{
	double retVal = 0;
	if((inVal & 0x8000) > 0)
	{
		// 负数
		retVal = (double)(0xffff - inVal + 1)* -1;
	}
	else
		retVal = (double)inVal;
...
static double kQ = 1.0f/1073741824.0f;
static double kG = (2500.0f/32767)*M_PI/180;
static double kA = (16.0f/32767)*9.8;
void CMPU_driver::m_ParseFrame(unsigned char *inBuf, int inLen)
{
	...
	fQuatW = m_CalQuaternionVal(&pBuf[0]) * kQ;
	fQuatX = m_CalQuaternionVal(&pBuf[4]) * kQ;
	fQuatY = m_CalQuaternionVal(&pBuf[8]) * kQ;
	fQuatZ = m_CalQuaternionVal(&pBuf[12]) * kQ;
	//printf("[CMPU_driver] qw= %d   qx= %d   qy= %d  qz= %d\n",nQuatW,nQuatX,nQuatY,nQuatZ);
	
	fGyroX = Short2Double(m_Piece2int(&pBuf[16]))*kG;
	fGyroY = Short2Double(m_Piece2int(&pBuf[20]))*kG;
	fGyroZ = Short2Double(m_Piece2int(&pBuf[24]))*kG;
	//printf("[CMPU_driver] gx= %d   gy= %d  gz= %d\n",nGyroX,nGyroY,nGyroZ);
	
	fAccX = Short2Double(m_Piece2int(&pBuf[28]))*kA;
	fAccY = Short2Double(m_Piece2int(&pBuf[32]))*kA;
	fAccZ = Short2Double(m_Piece2int(&pBuf[36]))*kA;
