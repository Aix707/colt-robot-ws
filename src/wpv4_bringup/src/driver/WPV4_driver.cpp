	...
	case 0x38:
		...
		case 0x08:		//Motor
			...
			case 0x61:	// 电机功率和速度
				for (i=0;i<4;i++)
				{
					arMotorPower[i] = m_ShortFromChar(&(inBuf[7+i*4]));
					arMotorSpeed[i] = m_ShortFromChar(&(inBuf[7+i*4+2]));
				}
				//printf("P[0]= %.4d P[1]= %.4d  P[2]= %.4d P[3]= %.4d \n", arMotorPower[0],arMotorPower[1],arMotorPower[2],arMotorPower[3]);
				//printf("S[0]= %.4d S[1]= %.4d  S[2]= %.4d S[3]= %.4d \n", arMotorSpeed[0],arMotorSpeed[1],arMotorSpeed[2],arMotorSpeed[3]);
				// printf("[Recv] ");
				break;
			}//end of switch(inBuf[5])
			// 显示接受到的16进制数据
